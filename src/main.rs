...
/// source of truth. Regenerated by `dx cartridge snapshot`; validated against
...
fn build_manifest() -> CapManifest {
    let mut all_caps: Vec<capdag::Cap> = serde_json::from_str(CAP_SNAPSHOTS)
        .expect("embedded cap-def snapshots must deserialize into Vec<Cap>; regenerate with `dx cartridge snapshot`");
